...
void head_shake(int x,int y){   //ส่ายหัว
   servo_head.write(x);
   delay(200);
   servo_head.write(100); 
   delay(200);
...
void head_turnleft(int x){      //ส่ายหัว ไปทางซ้าย  x คือค่าองศา ตั้งแต่ 0-100 
  if( 0 <= x >= 100){
   servo_head.write(x);
  }
}
void head_turnright(int x){   //ส่ายหัว ไปทางขวา  x คือค่าองศา ตั้งแต่ 100-180 
  if( 100 <= x >= 180){
   servo_head.write(x);
  }
}
